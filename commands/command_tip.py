...
class TipCommand(Command):
    ...
    COMMENT_EARN2TIP = (f'\n\n^(THIS IS A TEST TRANSACTION.)\n\n^(Learn more about Earn2Tip [HERE](https://www.reddit.com/r/EthTrader_Test/comments/17l2imp/introducing_earn2tip_and_the_new_tipping_bot/?utm_source=share&utm_medium=web2x&context=3).  NOTE: THIS IS A '
                        f'TEST TRANSACTION.)')
    ...
    def leave_comment_reply(self, is_earn2tip, comment, reply):
        if is_earn2tip:
            reply += self.COMMENT_EARN2TIP
    ...
    def process_command(self, comment):
        ...
        if re_result:
            ...
            if len(result) == 0:
                self.leave_comment_reply(False, comment, f"u/{comment.author.name} has not earn2tipped anyone this round")
                return
            ...
            self.leave_comment_reply(False, comment, tip_text)
            return
        ...
        if not user_address:
            ...
            self.leave_comment_reply(False, comment, f"Cannot tip u/{author} - you are not registered")
            return

        if not author_address:
            self.logger.info("author not registered")
            self.leave_comment_reply(False, comment, f"Cannot tip u/{author} - that user is not registered")
            return

        if user_address == author_address:
            self.logger.info("attempted self tipping")
            self.leave_comment_reply(False, comment, f"Sorry u/{comment.author.name}, you cannot tip yourself!")
            return
        ...
        if re_result:
            ...
            if amount == -1:
                self.leave_comment_reply(False, comment, f"Sorry u/{comment.author.name}, I could not process that number")
                return
            ...
            self.leave_comment_reply(True, comment,
        ...
        if re_result:
            ...
            amount = self.normalize_amount(re_result.group(1))
            if amount == -1:
                self.leave_comment_reply(True, comment, f"Sorry u/{comment.author.name}, I could not process that number")
                return
            ...
            if not token_meta:
                ...
                self.leave_comment_reply(False, comment,
        ...
        self.leave_comment_reply(False, comment, comment_reply)
